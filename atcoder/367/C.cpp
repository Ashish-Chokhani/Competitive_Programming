...
vector<vector<int>>textQueries(vector<string>sentences,vector<string>queries){
  map<string,multiset<int>>where;
  vector<vector<int>>ans;
  string word;
  char delimiter=' ';
  for(int i=0;i<sentences.size();i++){
    stringstream ss(sentences[i]);
    while(getline(ss,word,delimiter)) where[word].insert(i);
  }
  for(int i=0;i<queries.size();i++){
    ...
    while(getline(ss,word,delimiter)){
      if(first_word) for(auto it:where[word]) st.insert(it);
      else{
        multiset<int>nst=st;
        for(auto it:nst) if(where[word].find(it)==where[word].end()) st.erase(st.find(it));
      }
      first_word=false;
    }
    vector<int>ind;
    for(auto it:st) ind.push_back(it);
    if(!ind.size()) ind.push_back(-1);
    ans.push_back(ind);
  }
  return ans;
}
...
int32_t main(){
  vector<string>sentences(3),queries(4);
  for(int i=0;i<3;i++) getline(cin,sentences[i]);
  for(int i=0;i<4;i++) getline(cin,queries[i]);
