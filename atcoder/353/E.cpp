...
const int mod = 1000000001020030457;
const int INF = 1e18;
using ll = long long;
...
struct string_hash {
  ...
  void init(int n, string s, ll k = 89, ll m = 1000000001020030457) {
    mod = m;
    poly = k;
    prefix = vector<ll>(n);
    suffix = vector<ll>(n);
...
void solve(){
  ...
  for(int i=0;i<n;i++){
  	...
  	int sz=(int)s.length();
  	sh.init(sz,s);
  	for(int j=0;j<sz;j++) mp[sh.get_hash_prefix(0,j)]++;
  }
  for(auto it:mp) ans+=1LL*(it.second*(it.second-1))/2;
  cout<<ans<<endl;
} 
