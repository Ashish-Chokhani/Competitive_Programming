...
void solve(){
	...
	for(int i=0;i<M;i++){
		int T,P;
		cin>>T>>P;
		dp[T]=P-(T*C)+Sg[0].query(1,T).val;
		if(T+1<=N) dp[T]=max(dp[T],P+(T*C)+Sg[1].query(T,N).val);
		int curv1=Sg[0].query(T,T).val;
	    int curv2=Sg[1].query(T,T).val;
		Sg[0].update(T,T,dp[T]+(T*C)-curv1);
		Sg[1].update(T,T,dp[T]-(T*C)-curv2);
	}
	int ans=0;
