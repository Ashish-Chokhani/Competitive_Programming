...
void solve(){
	...
	for(int i=0;i<M;i++){
		...
		Bridges[i][0]=U; Bridges[i][1]=V; Bridges[i][2]=T;
	}
	...
	while(Q--){
	    ...
	    int ans=INF,cur=0,val=0;
	    do{
	    	for(int j=0;j<(1LL<<K);j++){
	    		int cur=0,val=0,nxt;
	    		for(int i=0;i<K;i++){
		    		if(j&1LL<<i) nxt=Bridges[B[i]][0];
		    		else nxt=Bridges[B[i]][1];
		    		val+=Bridges[B[i]][2];
		    		val+=dist[cur][nxt];
		    		cur=(nxt==Bridges[B[i]][0]?Bridges[B[i]][1]:Bridges[B[i]][0]);
	    	    }
		    	assert(cur<N);
		    	val+=dist[cur][N-1];
		    	ans=min(ans,val);
