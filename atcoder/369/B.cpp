...
void solve(){
  ...
  for(int i=0;i<n;i++){
  	int k; cin>>k;
  	char ch; cin>>ch;
  	if(ch=='L') val[0].push_back(k);
  	else val[1].push_back(k);
  }
  	int ans=0;
  	for(int i=1;i<val[0].size();i++) ans+=abs(val[0][i]-val[0][i-1]);
  	for(int i=1;i<val[1].size();i++) ans+=abs(val[1][i]-val[1][i-1]);
  	cout<<ans<<endl;
} 
