...
void DFS(string &node,ll k){
	stack<pair<char,string>>st;
	st.push({'2',node});
	while(!st.empty()){
		//debug(st);
		string node=st.top().second;
		char wt=st.top().first;
		if(outdegree[node]==2){
			st.pop();
			if(wt!='2') edges.push_back(wt);
		}else{
			if(outdegree[node]==0){
				st.push({'0',node.substr(1)+'0'});
			}
			else{
				st.push({'1',node.substr(1)+'1'});
			}
			outdegree[node]++;
		}
	}
...
void solve(){
  ...
  for(ll i=0;i<n-1;i++){
  	edges.push_back(starting_node[i]);
  }
  DFS(starting_node,2);
  reverse(edges.begin()+n-1,edges.end());
  for(char &ch:edges) cout<<ch;
  cout<<endl;
} 
