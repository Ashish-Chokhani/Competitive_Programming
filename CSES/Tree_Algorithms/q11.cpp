...
void solve(){
  ...
  while(q--){
    ...
    if(type==1){
      ...
      cin>>s>>x;
      --s;
      ll prev_val_pos=arr[start[s]];
      arr[start[s]]=x;
      ll prev_val_neg=arr[en[s]];
      arr[en[s]]=-x;
      Sg.update(start[s],start[s],x-prev_val_pos);
      Sg.update(en[s],en[s],-x-prev_val_neg);
    }
    else{
      ll s;
      cin>>s;
      --s;
      ll val=Sg.query(start[0],start[s]).val;
      cout<<val<<endl;
    }
  }
} 
