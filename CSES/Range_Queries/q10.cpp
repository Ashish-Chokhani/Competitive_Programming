...
struct segtree {
  ...
  ll findElements(int i,int tl, int tr,int val) {
    ll value=tree[i].val;
    if(value>val) return 0;
    if(tl==tr) return 1;
    
    int mid = (tl + tr) / 2;
    ll a = findElements(2*i+1,tl,mid,val),
       b = findElements(2*i+2,mid+1,tr,val);
    return a+b;
  }
...
void solve(){
  ...
  while(q--){
    ...
    if(type=='?'){
      ll a,b;
      cin>>a>>b;
      ll val1=o_st.order_of_key(b+1);
      ll val2=o_st.order_of_key(a);
      cout<<-val2+val1<<endl;
    }
    else{
      ll k,x;
      ...
      ll cur_val=salary[k];
      salary[k]=x;
      o_st.erase(o_st.find_by_order(o_st.order_of_key(cur_val)));
      o_st.insert(x);
    }
  }
 }
