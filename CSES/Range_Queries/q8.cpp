...
struct segtree {
  ...
  ll findIndex(int i,int tl, int tr,int val) {
    ll value=tree[i].val;
    ...
    int mid = (tl + tr) / 2;
    ll a = tree[2*i+1].val;
    if(a>=val) return findIndex(2*i+1,tl,mid,val);
    return findIndex(2*i+2,mid+1,tr,val);
  }
...
void solve(){
  ...
  Sg.init(n,rooms);
  for(ll i=0;i<m;i++){
  	ll ind=Sg.findIndex(0,0,n-1,group[i]);
  	cout<<ind+1<<" ";
  	if(ind!=-1) Sg.update(ind,ind,-group[i]);
  }
  cout<<"\n";
 }
