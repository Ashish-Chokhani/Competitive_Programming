...
void solve(){
  ...
  while(q--){
  	...
  	if(type==1){
  		...
  		ll prev_val=arr[k];
  		arr[k]=u;
  		Sg[0].update(k,k,u-prev_val);
      Sg[1].update(k,k,u-prev_val);
  	}
  	else{
  		ll k;
  		cin>>k;
      ...
      ll val1=INF,val2=INF;
  		if(k>=1) val1=Sg[0].query(0,k-1).val;
  		if(k+1<n) val2=Sg[1].query(k+1,n-1).val;
  		cout<<min(val1+k,min(arr[k],val2-k))<<endl;
  	}
  }
} 
