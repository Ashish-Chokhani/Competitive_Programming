...
struct string_hash {
  ...
  void init(int n, string s, ll k = 89, ll m = primes[6]) {
    mod = m;
    poly = k;
    prefix = vector<ll>(n);
    suffix = vector<ll>(n);
...
void solve(){
  ...
  sh[0].init(n,s);
  sh[1].init(n,t);
  //Handle special case when p==n-1
  string cand1="",cand2="";
  ...
  sh[2].init(n,cand1);
  sh[3].init(n,cand2);
  auto isEqual=[&](int i,int p,int h1,int h2){
  	int h3=sh[i].get_hash_prefix(0,n-2-p);
  	int h4=sh[i].get_hash_prefix(n-1-p,n-1);
  	return h1==h3 && h2==h4;
  };
  for(int p=0;p<n-1;p++){
  	int h1=sh[0].get_hash_prefix(p+1,n-1);
  	int h2=sh[1].get_hash_prefix(n-1-p,n-1);
  	if(isEqual(2,p,h1,h2) || isEqual(3,p,h1,h2)){
  		cout<<p+1<<endl;
  		return;
  	}
  }
