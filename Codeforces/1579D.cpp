...
int main() {
	...
	for(int i = 0; i < q; i++) {
        ...
        int left=-1,right=-1,maxi_depth=-1;
        for(int j=0;j<k;j++){
        ...
        for(int j=0;j<k;j++){
            int node=arr[j];
            int LCA=lca.get_lca(node,left);
            if(LCA!=node && lca.depth[node]>maxi_depth)
            {
                maxi_depth=lca.depth[node];
                right=node;
            }
        }
        if(right==-1)
        ...
        int flag=0;
        for(int j=0;j<k;j++){
            if(lca.get_dist(left,arr[j])+lca.get_dist(right,arr[j])!=lca.get_dist(left,right)){
                cout<<"NO\n";
                flag=1;
                break;
            }
