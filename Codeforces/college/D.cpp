...
void solve(){
  ...
  for(int i=0;i<3;i++) for(int j=1;j<=n;j++) pre[i][j]=pre[i][j-1]+(arr[j]>>i&1);
  debug(pre)
  for(int i=0;i<3;i++) for(int j=1;j<=n;j++) pre[i][j]=pre[i][j-1]+(pre[i][j]&1);
  int ans=0;
  for(int i=1;i<=n;i++){
  	int msb=63-__builtin_clzll(arr[i]);
  	int l1=pre[msb][i-1],r1=pre[msb][n]-pre[msb][i-1];
  	int l2=i-l1,r2=n-i+1-r1;
  	ans+=l1*r1+l2*r2;
  }
  cout<<ans<<endl;
} 
