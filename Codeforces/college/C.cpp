...
void solve(){
  int n,k;
  cin>>n>>k;
  unordered_map<int,int,custom_hash>isPresent;
  int uniques=0;
  for(int i=0;i<k;i++){
  	int u,v;
  	cin>>u>>v;
  	if(!isPresent[u]) uniques++;
  	isPresent[u]=1;
  	if(!isPresent[v]) uniques++;
  	isPresent[v]=1;
  }
  int m=n-uniques;
  Mint ans=0;
  for(int i=0;i<=m;i++){
  	if((m-i)&1) continue;
  	Mint val=modop::nck(m,i);
  	val*=modop::fact[m-i];
    val*=modop::ifact[(m-i)/2];
  	ans+=val;
  }
  cout<<ans<<endl;
} 
