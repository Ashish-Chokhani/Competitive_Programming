...
void solve(){
  ...
  vector<array<int,3>>edges,arr(n);
  int idx=0;
  for(auto &[l,r,a]:arr){
    cin>>l>>r>>a;
    segments.push_back({l,0,a,idx});
    segments.push_back({r,1,a,idx++});
  }
  sort(all(segments));
  set<pair<int,int>>st;
  for(auto &[_,t,a,idx]:segments){
    if(t==0){
        auto it=st.upper_bound({a,0});
        if(it!=st.end()) edges.push_back({it->first-a,it->second,idx});
        if(it!=st.begin()){
            it=prev(it);
            edges.push_back({a-it->first,it->second,idx});
        }
        st.insert({a,idx});
    }
    else st.erase({a,idx});
  ...
  for(auto &[wt,u,v]:edges){
    if(!(fast_dsu::connected(u,v))){
        mst_edges++;
        ans+=wt;
        fast_dsu::merge(u,v);
    }
  }
  if(mst_edges!=n-1) ans=-1;
  cout<<ans<<endl;
} 
