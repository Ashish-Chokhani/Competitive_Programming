...
void solve(){
  int n;
  cin>>n;
  vector<int>sieve(n+1,1),primes,ans;
  for(int i=2;i*i<=n;i++) if(sieve[i]) for(int j=2*i;j<=n;j+=i) sieve[j]=0;
  for(int i=2;i<=n;i++) if(sieve[i]) primes.push_back(i);
  int k=1;
  while(n-1>(k%2==1?k*(k+1)/2:k*k/2+1)) k++;
  vector<vector<int>>g(k,vector<int>(k,1));
  if(k%2==0) for(int i=1;i+1<k;i+=2) g[i][i+1]=g[i+1][i]=0;
