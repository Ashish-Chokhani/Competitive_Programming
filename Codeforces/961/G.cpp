...
void solve(){
  ...
  auto cntTriplets=[&](int l,int r)->int{
  	vector<vector<int>>dp1(N+1,vector<int>(2));
  	ordered_set st[2];
  	for(int i=l;i<=r;i++){
  		dp1[i][0]=st[0].order_of_key(arr[i]);
  		st[0].insert(arr[i]);
  	}
  	for(int i=r;i>=l;i--){
  		dp1[i][1]=st[1].order_of_key(arr[i]);
  		st[1].insert(arr[i]);
  	}
  	int val=0;
  	for(int i=l;i<=r;i++) val+=dp1[i][0]*dp1[i][1];
  	return val;
  };
  int ans=0;
  for(int j=0;j<=N;j++) dp[N][j]=1;
  for(int i=N-1;i>=1;i--) for(int j=0;j<i;j++){
  	int triplets=cntTriplets(j+1,i);
  	for(int k=i+1;k<=N;k++) if(cntTriplets(i+1,k)>triplets) dp[i][j]+=dp[k][i];
  }
