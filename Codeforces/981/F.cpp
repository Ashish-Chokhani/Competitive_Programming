...
struct string_hash {
  ...
  void init(int n, vector<int>& s, ll k = 89, ll m = 1000000007) {
    mod = m;
    poly = k;
    prefix = vector<ll>(n);
    suffix = vector<ll>(n);
...
void solve(){
	...
	for(int i=1;i<=2*K;i++){
		...
		f1%=mod; f0%=mod;
		S.push_back(f1%K);
		if(f1%K==0) A.push_back(i+2);
		
	}
	int period=K+2;
	for(int i=K+1;i>=1;i--){
		vector<int>t1,t2;
		for(int j=0;j<i;j++) t1.push_back(S[j]);
		for(int j=i;j<2*i;j++) t2.push_back(S[j]);
		if(t1==t2){
			period=i;
			break;
		}
	...
	debug(A);
	int zeros=0;
	for(int i=0;i<A.size() && A[i]<=period;i++) zeros++;
	cout<<period<<" "<<zeros<<endl;
	assert(period<=K+1);
	assert(zeros!=0);
	int ans;
    if(N%zeros==0){
    	ans=max(0LL,(max(0LL,(N-zeros))/zeros)*period);
    	ans+=A[zeros-1];
    }
    else{
    	ans=(N/zeros)*period;
