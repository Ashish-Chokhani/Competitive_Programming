...
struct string_hash {
  ...
  void init(int n, string s, ll k = kk[5], ll m = primes[1]) {
    mod = m;
    poly = k;
    prefix = vector<ll>(n);
    suffix = vector<ll>(n);
...
void solve(){
	...
	cin>>s;
	int N=s.length();
	//handle all a's separately
	int cnt_a=0;
	for(int i=0;i<N;i++) cnt_a+=s[i]=='a';
	if(cnt_a==N){
		cout<<N-1<<endl;
		return;
	}
	...
	string_hash sh;
	sh.init(N,s);
	auto isValid=[&](int len,int h){
		int j=a[0];
		while(j<N){
			if(j+len-1>=N) return false;
			int h1=sh.get_hash_prefix(j,j+len-1);
			if(h1!=h) return false;
			j+=len;
			if(j!=N && j+a[j]!=N) m=min(m,a[j]);
			j=j+a[j];
		}
		return true;
	};
	int ans=0;
	for(int len=1;a[0]+len<=N;len++){
		int h=sh.get_hash_prefix(a[0],a[0]+len-1);
		m=a[0];
		if(isValid(len,h)) ans+=m+1;
	}
	cout<<ans<<endl;
} 
