...
struct Matrix {
    ...
    Matrix(int size): m(size),A(size,vector<Mint>(size)) {}
...
void solve(){
  int m,n;
  cin>>m>>n;
  vector<Mint>s(m),l(m),t(m);
  for(Mint &x:s) cin>>x;
  for(Mint &x:l) cin>>x;
  for(int i=0;i<m;i++) t[i]=s[i]+l[i];
  Matrix mat(m),result(m);
  for(int i=0;i<m;i++) for(int j=0;j<m;j++){
  	mat.A[i][j]=t[i]*t[j]-l[i]*l[j];
  	if(i==j) result.A[i][j]=1;
  }
  while(n>0){
  ...
  Mint ans=0;
  for(int i=0;i<m;i++) ans+=result.A[i][0];
  cout<<ans<<endl;
} 
int32_t main()
{
