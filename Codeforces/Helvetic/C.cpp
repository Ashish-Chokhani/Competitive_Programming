...
void solve(){
  ...
  vector<int>diff(n,0),order(n);
  for(int i=1;i<n;i++) diff[i]=arr[i-1]-arr[i];
  iota(all(order),0);
  sort(all(order),[&](int i,int j){
  	return diff[i]<=diff[j];
  });
  vector<int>ans(n),done(n+1),vis(n);
  int r=n,l=1;
  for(auto it:order){
  	if(it==0 || it==n-1 || vis[it] || vis[it+1] || vis[it-1]) continue;
  	if(l+2>=r) break;
  	ans[it]=r--;ans[it-1]=l++;ans[it+1]=l++;
  	done[r+1]=done[l-1]=done[l-2]=1;
  	vis[it]=vis[it-1]=vis[it+1]=1;
  }
  int j=1;
  debug(ans);
  debug(done)
