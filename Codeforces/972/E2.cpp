...
void solve(){
  ...
  cin>>l>>n>>m;
  vector<int>A(l);
  vector<vector<int>>B(n+1,vector<int>(m+1)),dp(n+1,vector<int>(m+1));
  for(int i=0;i<l;i++) cin>>A[i];
  for(int i=0;i<n;i++) for(int j=0;j<m;j++) cin>>B[i][j];
  for(int i=l-1;i>=0;i--){
    vector<vector<int>>C(n+1,vector<int>(m+1));
    for(int j=0;j<n;j++) for(int k=0;k<m;k++) if(B[j][k]==A[i] && !dp[j+1][k+1]) C[j][k]=1;
    for(int j=n-1;j>=0;j--) for(int k=m-1;k>=0;k--) dp[j][k]=max({dp[j+1][k],dp[j][k+1],C[j][k]});
  }
  debug(dp)
  cout<<(dp[0][0]?"T\n":"N\n");
} 
