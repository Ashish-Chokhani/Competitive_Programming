...
#define ran(i, a, b) for (auto i = (a); i < (b); i++)
  ...
  struct ModArithm {
    ...
    ull multf(ull a, ull b) {
        ull mult = (long double)a * b * rec + 0.5L;
      ll res = a * b - mult * n;
      if (res < 0) res += n;
      return res; // in [0, n-1)
    }
...
void solve(){
	...
	cin>>s>>t;
	map<int,char>id;
	id[0]='A';id[1]='G';id[2]='C';id[3]='T';
	auto get_vector=[&](int idx){
		vector<int>a(n+k),b(m);
		int last=-INF;
		for(int i=0;i<n+k;i++){
			if(i<n && s[i]==id[idx]) last=i;
			if(i-last<=2*k) a[i]=1;
		}
		int cnt=0;
		for(int i=0;i<m;i++){
			if(t[i]!=id[idx]) continue;
			cnt++;
			b[i]=1;
		}
		reverse(all(b));
		vector<int>ans(n+m+k-1);
		FFT::mult_poly_mod(a,b,ans);	
		for(int i=0;i<n+m+k-1;i++) ans[i]=ans[i]>=cnt;
		return ans;
	};
	vector<int>A=get_vector(0);
	vector<int>C=get_vector(1);
	vector<int>G=get_vector(2);
	vector<int>T=get_vector(3);
	int ans = 0;
    for (int i = 0; i <= n - m; i++) {
    	int p = i + k+ m - 1;
        if (A[p] && C[p] && G[p] && T[p]) ans++;
    }
    cout << ans << endl;
} 
