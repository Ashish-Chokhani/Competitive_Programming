...
void solve(){
	...
	int n=s.length();
	vector<int>dp(n+1);
	for(int i=1;i<=n;i++) dp[i]+=dp[i-1]+(s[i-1]=='1'?1:-1);
	unordered_map<int,vector<int>,custom_hash>where,where1;
    for(int i=1;i<=n;i++) where[dp[i]].push_back(i);
    where1=where;
    for(int j=-n;j<=n;j++){
    	int sz=where[j].size();
    	for(int k=sz-1;k>=0;k--) where1[j][k]=(n-where[j][k]+1)+(k+1<sz?where1[j][k+1]:0LL);
    }
    Mint ans=0;
    for(int i=1;i<=n;i++){
    	int id=lower_bound(all(where[dp[i-1]]),i+1)-where[dp[i-1]].begin();
    	int sz=where[dp[i-1]].size();
    	if(id==sz) continue;
    	Mint l=i,r=where1[dp[i-1]][id];
    	ans+=l*r;
    }
    cout<<ans<<endl;
} 
