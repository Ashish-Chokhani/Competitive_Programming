...
void solve(){
  ...
  function<void(int,int,int)>DFS=[&](int u,int p,int l){
  	...
  	for(int v:adj[u]){
  		...
  		for(auto it:st[v]) st[u].insert({it.first,it.second});
  	}
  	if(nchild==0) st[u].insert({l,INF});
  	else{
  		int diff=s-a[u];
  		if(diff>0) st[u].insert({l,diff});
  		else{
  			int need=-diff;
  			while(need>0){
  				auto it=st[u].begin();
  				int h=(*it).first;
  				int val=min(need,(*it).second);
  				assert(val>0 && h>l);
  				ans+=(h-l)*val;
  				need-=val;
  				int rem=(*it).second-val;
  				st[u].erase(it);
  				if(rem>0) st[u].insert({h,rem});
  			}
  		}
  	}
  };
