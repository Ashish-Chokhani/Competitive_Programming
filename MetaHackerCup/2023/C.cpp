...
public:
  ...
  void ins(string x) {
    ...
    for (int i=0;i<x.length();i++) {
        char ch=x[i];
        int nxt;
        if(ch>='A' && ch<='Z') nxt=ch-'A';
        else nxt=26;
        if(mp[i]) nxt=26;
        targ = targ->next(nxt);
    }
    Mint val=cnt;
    Mint v=val+(26*(modop::mpow(26,wildcards)-1)/25);
    cout<<"F "<<v<<endl;
    ans+=v;
    cout<<"Ok "<<ans<<" "<<val<<" "<<wildcards<<endl;
  }
};
...
void solve(){
    ...
    for(int i=0;i<N;i++){
        cin>>A[i];
        for(int j=0;j<A[i].length();j++) if(A[i][j]=='?') mp[j]=1;
    }
    Trie T;
    for(int i=0;i<N;i++) T.ins(A[i]);
    cout<<ans<<endl;
} 
